            ...
            <div class="row">
              <div class="col-12 col-md-3"
                ...
                <div class="form-group" style="margin-bottom: unset">
                  <select class="form-control" id="filter" v-if="filter.type==='dropdown'" v-model="filter.value" name="rider">
                    <option value="" readonly>{{ filter.title }}</option>
                    <option :value="option.value" v-for="(option,opIndex) in filter.options" :key="opIndex">
                      {{ option.text }}
                    </option>
                  </select>
                  <date-picker v-else-if="filter.type==='datepicker'" v-model="filter.value" :placeholder="filter.title" format="DD-MM-YYYY" valueType="format"></date-picker>
                  <date-picker v-else-if="filter.type==='rangepicker'" v-model="filter.value" :placeholder="filter.title" format="DD-MM-YYYY" range valueType="format"></date-picker>
                </div>
              </div>
              <div class="col-12 col-md-1 ml-1">
                <div class="form-group" style="margin-bottom: unset">
                  <button @click="readData" class="btn btn-success"><i class="mdi mdi-filter"></i>Filter</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="datatable scrollable" style="overflow-x:auto">
              <table id="datatable scrollable" class="table table-bordered table-striped dt-responsive nowrap dataTable no-footer dtr-inline table-sm small"
          ...
          (response) => {
            if (type !== 'export') {
              ...
              if (this.dataSets.length > 0) {
                ...
                this.showTo = response.meta.to;
                this.showFrom = response.meta.from;
